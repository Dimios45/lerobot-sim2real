"""
This script is used to evaluate a random or RL trained FPO agent on a real robot using the LeRobot system.
"""
...
from lerobot_sim2real.rl.fpo_rgb import FlowPolicy
...
class Args:
    ...
    sampler_steps: int = 8
    """Number of Euler steps for flow sampler from tau=1->0"""
...
def main(args: Args):
    ...
    agent = FlowPolicy(sim_env, sample_obs=real_obs, sampler_steps=args.sampler_steps)
    if args.checkpoint:
